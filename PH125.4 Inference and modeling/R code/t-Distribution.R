# Calculate the probability of seeing t-distributed random variables being more than 2 
...
# Make a function called 'pt_func' that calculates the probability that a value is 
# more than |2| for any degrees of freedom 
...
# Create variables for the mean height 'mu', the sample size 'N', and the number of 
# times the simulation should run 'B'
mu <- mean(x)
N <- 15
B <- 10000
...
res <- replicate(B,{
  ...
  interval <- c(mean(s)-qt(.975,N-1)*sd(s)/sqrt(N),mean(s)+qt(.975,N-1)*sd(s)/sqrt(N))
  between(mu, interval[1], interval[2])
})
